...
void Oculars::init()
{
	...
	try {
        ...
        int ocularCount = settings->value("ocular_count", 1).toInt();
...
void Oculars::initializeActivationActions()
{
    ...
    actionShowTelrad = addAction("actionShow_Telrads", ocularsGroup, N_("Telrad sight"), "enableTelrad", "Ctrl+B");
...
//    QString ocularIniPath = StelFileMgr::findFile("modules/Oculars/", flags) + "ocular.ini";
	...
	{
        QFile src("assets:/data/default_ocular.ini");
        if (!src.copy(StelFileMgr::getUserDir()+"/modules/Oculars/ocular.ini"))
		{
